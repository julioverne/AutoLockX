...
@interface AutoLockXSettingsController : PSListController
{
	UILabel* _label;
	UILabel* underLabel;
}
...
@implementation AutoLockXSettingsController
- (id)specifiers {
	if (!_specifiers) {
		...
		spec = [PSSpecifier preferenceSpecifierNamed:@"Timeout To Lock (Minutes)"
						      target:self
											  set:Nil
											  get:Nil
					      detail:Nil
											  cell:PSGroupCell
											  edit:Nil];
		[spec setProperty:@"Timeout To Lock (Minutes)" forKey:@"label"];
		[specifiers addObject:spec];
		
		spec = [PSSpecifier preferenceSpecifierNamed:@"Minutes"
                                                  target:self
													 set:@selector(setPreferenceValue:specifier:)
													 get:@selector(readPreferenceValue:)
												  detail:Nil
													cell:PSEditTextCell
													edit:Nil];
		[spec setProperty:@"minutes" forKey:@"key"];
 		[spec setProperty:@"5" forKey:@"default"];
		[specifiers addObject:spec];
		
		spec = [PSSpecifier preferenceSpecifierNamed:@"When Lock"
						      target:self
											  set:Nil
											  get:Nil
					      detail:Nil
											  cell:PSGroupCell
											  edit:Nil];
		[spec setProperty:@"When Lock" forKey:@"label"];
		[specifiers addObject:spec];
		
		spec = [PSSpecifier preferenceSpecifierNamed:@"Stop Media"
                                                  target:self
											         set:@selector(setPreferenceValue:specifier:)
											         get:@selector(readPreferenceValue:)
                                                  detail:Nil
											        cell:PSSwitchCell
											        edit:Nil];
		[spec setProperty:@"stopMedia" forKey:@"key"];
        ...
        [specifiers addObject:spec];
		spec = [PSSpecifier emptyGroupSpecifier];
        [spec setProperty:@"AutoLockX © 2021" forKey:@"footerText"];
        [specifiers addObject:spec];
...
- (void)love
{
	...
	[twitter setInitialText:@"#AutoLockX by @ijulioverne is cool!"];
	if (twitter != nil) {
...
- (void)reset
{
	...
	notify_post("com.julioverne.autolockx/SettingsChanged");
...
- (void)HeaderCell
{
	@autoreleasepool {
		...
		[underLabel setText:@"Force Lock Device 🔓"];
